...
function AssistantBubble({ message }: { message: ChatMessage }) {
  return (
    <div className="max-w-[92%] rounded-none border border-dashed border-slate-200 bg-white px-3.5 py-3 text-sm leading-relaxed text-slate-800 shadow-sm">
      {message.focus ? (
        <span className="mb-2 inline-flex rounded-none bg-slate-100 px-2.5 py-1 text-[11px] font-medium text-slate-500">
          {message.focus}
        </span>
      ) : null}
      {message.headline ? (
      ...
      {message.text.split("\n\n").map((paragraph, index) => (
        <p key={`${message.id}-p-${index}`} className={cn(message.headline || index > 0 ? "mt-2" : undefined)}>
          {paragraph}
        </p>
      ))}
    </div>
...
export default function PortfolioChatPanel() {
  ...
  return (
    <TooltipProvider delayDuration={150}>
      <div className="portfolio-panel-frame h-[32rem] w-full min-h-0 overflow-hidden">
        <span
          aria-hidden
        ...
        />
        <Card className="relative z-[1] flex h-full min-h-0 w-full flex-col gap-0 overflow-hidden rounded-none border border-dashed border-slate-200 bg-slate-50 shadow-sm">
        <CardHeader className="shrink-0 gap-1 portfolio-border-b bg-slate-50 px-4 py-4">
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  ...
                  variant="outline"
                  size="icon"
                  className="h-8 w-8 shrink-0 rounded-none border border-dashed border-slate-200 bg-white text-slate-900 hover:bg-slate-900 hover:text-white"
        ...
        <CardContent className="relative min-h-0 flex-1 overflow-hidden bg-slate-50 p-0">
          {messages.length === 0 ? (
            <div className="flex h-full flex-col items-center justify-center gap-4 overflow-y-auto px-4 py-4 text-center">
              ...
              >
                <div className="flex flex-col gap-4">
                  {messages.map((message) => (
                    ...
                    >
                      {message.role === "user" ? (
                        <div className="max-w-[88%] rounded-none bg-slate-900 px-3.5 py-2.5 text-sm leading-relaxed text-white">
                          {message.text}
                        </div>
                      ) : (
              ...
              {showScrollButton ? (
                <div className="pointer-events-none absolute inset-x-0 bottom-3 flex justify-center">
                  <Button
                    ...
                    size="sm"
                    variant="secondary"
                    className="pointer-events-auto h-8 rounded-full bg-white text-slate-900 shadow-md hover:text-slate-900"
                    onClick={() => {
                      pinnedToBottomRef.current = true
                      scrollToBottom()
                      setShowScrollButton(false)
        ...
        <CardFooter className="shrink-0 portfolio-border-t bg-slate-50 p-3">
          <form
            className="w-full"
            onSubmit={(event) => {
          ...
          >
            <div className="cursor-target border border-dashed border-slate-200 bg-white focus-within:border-slate-900">
                <textarea
                  value={input}
